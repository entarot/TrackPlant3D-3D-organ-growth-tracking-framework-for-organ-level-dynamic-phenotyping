...
<img src="图1v3.jpg" width="100%" height="50%"><br><br>
...
- Folder **`[data]`**  contains four subfolders:
  ...
  - `./data/sampled_data`: Contains the downsampled point clouds processed by 3DEPS.
    - `core`: Stores the non-edge point clouds extracted from the point cloud.
    - `edge`: Stores the edge point clouds extracted from the point cloud.
    - `merge`: Stores the merged point cloud with 8192 points.
    - `3DEPS`: Stores the downsampled point cloud data after 3DEPS (each with 256 points), sorted by species.
  - `./data/registration_result`: Contains the registration results generated by CPD, sorted by species.
  - `./data/tracking_result`: Contains the tracking results produced by TrackPlant3D, sorted by species.
